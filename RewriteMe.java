...
public class RewriteMe {
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        return questions.stream().flatMap(b -> b.getAllAnswers().stream()).distinct().collect(Collectors.toList());
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().flatMap(b -> b.getAllAnswers().stream()).anyMatch(s -> s.equals(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) questions.stream().flatMap(value -> value.getAllAnswers().stream()).filter(s -> s.equals(answerCandidate)).count();
    ...
    {
        return questions.stream().filter(s -> s.getCategory().equals(c)).flatMap(v -> v.getAllAnswers().stream())
                .reduce((string1,string2) -> string1.length() >= string2.length() ? string1 : string2).orElse("0");
    }
